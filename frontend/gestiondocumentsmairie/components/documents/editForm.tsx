...
export default function CreateDocumentForm() {
   ...
   const [doc, setDoc] = useState<any>(null)
    ...
    useEffect(() => {
      const fetchDoc = async () => {
        ...
        //   // Préremplir les selects dynamiques si données présentes
        //   if (res && Array.isArray(res[0]?.documentPersonnes)) {
        //     console.log("Document id:", res[0].documentPersonnes)
        //     const initialValues = {};
        //     res[0].documentPersonnes.forEach((documentPersonne: any) => {
        //       // On utilise documentPerso pour obtenir la valeur à préremplir (id de la personne)
        //       const valeur = documentPerso(documentPersonne);
              
        //       if (valeur) {
        //         initialValues[documentPersonne.fonction] = valeur;
        //       }
        //     });
        ...
        res.map( (documentPers: any) => {
          const initialValues = {};
          const valeur = documentPerso(documentPers)
          if (valeur) {
            initialValues[documentPers.fonction] = valeur;
          }
          setVariableValues(initialValues);
        })
       } catch (error) {
 ...
 async function fetchAllPersonnes() {
  ...
  // Correction de la fonction documentPerso pour retourner l'id de la personne liée
  async function documentPerso(documentPersonne: any) {
    try {
      if (documentPersonne.fonction.includes(".")) {
        const prop = documentPersonne.fonction.split(".")[1];
        const pers = Array.isArray(documentPersonne.personne)
          ? documentPersonne.personne.find((p: any) => p.id === documentPersonne.personneId)
          : documentPersonne.personne;
          console.log("Valeur pour la fonction", documentPersonne.fonction, ":", pers[prop]);
        return pers ? pers[prop] : "";
      } else {
        const pers = Array.isArray(documentPersonne.personne)
          ? documentPersonne.personne.find((p: any) => p.id === documentPersonne.personneId)
          : documentPersonne.personne;
        return pers["nom"] + " " + pers["prenom"]; // Retourne le nom et prénom de la personne
      }
    } catch (error) {
      return "";
    }
                    ...
                    {templates.map((template) => (
                      <SelectItem key={template.id} value={template.id?.toString?.() || ""}>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-slate-900">{doc[0].document.template.typeDocument}</span>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex gap-3">
                                <div className="flex-1">
                                  ...
                                  {/* Affichage de la personne sélectionnée */}
                                  {variableValues[variable] && (
                                    (() => {
                                      const selected = personnes.find(p => p.id.toString() === variableValues[variable]);
                                      if (selected) {
                                        return (
                                          <div className="mt-2 text-green-700 text-sm flex items-center gap-2">
                                            <CheckCircle className="h-4 w-4 text-green-500" />
                                            Sélectionné : <span className="font-semibold">{selected.prenom} {selected.nom}</span>
                                          </div>
                                        );
                                      } else {
                                        return (
                                          <div className="mt-2 text-red-600 text-sm flex items-center gap-2">
                                            ...
                                            La personne sélectionnée n'existe plus dans la liste.
                                          </div>
                                        );
                                      }
                                    })()
